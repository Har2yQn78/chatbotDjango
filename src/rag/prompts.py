...
custom_text_to_sql_prompt = PromptTemplate("""Given an input question, generate an optimized {dialect} PostgreSQL query. 

Follow these guidelines:
1. Use only tables and columns from the provided schema
2. Select only relevant columns needed to answer the question
3. Use appropriate JOINs, filtering conditions, and aggregations
4. Handle text matching with ILIKE for case-insensitivity
5. Order results in a way that best answers the question
6. Consider alternative phrasings, synonyms, and terminology variations
7. Optimize for both correctness and performance

Your response should contain only the SQL query with no explanations, comments, or markdown.
